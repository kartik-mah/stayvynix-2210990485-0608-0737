...
export const addListing = async (req,res) => {
    try {
        let host = req.userId;
        let {title,description,rent,city,landMark,category,latitude,longitude} = req.body
        const files = req.files || {};
        const image1File = files?.image1?.[0];
        const image2File = files?.image2?.[0];
        const image3File = files?.image3?.[0];

        if (!image1File || !image2File || !image3File) {
            return res.status(400).json({ message: "All three images are required" });
        }

        // Fallback to local multer path when Cloudinary is unavailable.
        let image1 = (await uploadOnCloudinary(image1File.path)) || image1File.path;
        let image2 = (await uploadOnCloudinary(image2File.path)) || image2File.path;
        let image3 = (await uploadOnCloudinary(image3File.path)) || image3File.path;
...
export const updateListing = async (req,res) => {
    try {
        ...
        const files = req.files || {};
        if(files.image1){
        image1 = (await uploadOnCloudinary(files.image1[0].path)) || files.image1[0].path}
        if(files.image2)
        {image2 = (await uploadOnCloudinary(files.image2[0].path)) || files.image2[0].path}
        if(files.image3){
        image3 = (await uploadOnCloudinary(files.image3[0].path)) || files.image3[0].path}
